...
module PortfolioAnalyzer
  ...
  def self.handle_view_images(img_download_dir, mahara_accessor, portfolio_view)
    ...
    portfolio_view.uploaded_images.each do |image|
      ...
      begin
        ...
        $stderr.say "An error of type #{ex.class} happened, message is #{ex.message}"
      end
      ...
      begin
        ...
        $stderr.say "error identifying image type: " + e.to_s
      end
      ...
      nokogiri_doc.search(tags.keys.join(',')).each do |node|
        ...
        if (src == image.src) then
          say "adapting view image #{src} to #{image_download_path}"
  ...
  def self.add_to_solr(member, portfolio_view, solr)
    say "adding view to Solr: #{portfolio_view.title} ... " unless solr == nil
    begin
      solr.add portfolio_view.to_solr unless solr == nil
    rescue Exception => e
      $stderr.say "ERROR adding document to solr: " + e.to_s
      e.backtrace.each do |bt|
        $stderr.say "   " + bt
      end

      say "view size: #{portfolio_view.text.length}"
    end

    # solr.add :id=>1, :price=>1.00 unless solr == nil
    say "done!" unless solr == nil
  end
  ...
  def self.read_user_config(download_dir)
    ...
    begin
      ...
      $stderr.puts "Could not access user config file: #{err}"
    end
    user_names
  end
