...
# ddExtracted from https://github.com/premailer/premailer/blob/master/lib/premailer/html_to_plain_text.rb
...
module HtmlToPlainText
  ...
  def self.convert_to_text(html, line_length = 65, from_charset = 'UTF-8')
    txt = html
...
def doit
  ...
  data = file.read
  file.close
  text = Sanitize.clean(data, remove_contents: ['script', 'style'])
  puts text
  puts
  puts "*************************************************************************************************"
  puts HtmlToPlainText.convert_to_text(text)
end